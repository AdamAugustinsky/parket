...
enum TilerTests {
    ...
    static func runAll() -> (passed: Int, failed: Int) {
        ...
        do {
            ...
            let masterInset = floor(Config.shared.windowGap / 2)
            let stackInset = Config.shared.windowGap - masterInset
            check(frames.count == 2, "two windows count")
            check(frames[0].width == masterWidth - masterInset, "master width leaves split gap")
            check(frames[1].origin.x == masterWidth + stackInset, "stack x leaves split gap")
            check(frames[1].width == 1920 - masterWidth - stackInset, "stack width leaves split gap")
            check(frames[1].origin.x - frames[0].maxX == Config.shared.windowGap, "split gap width")
        ...
        do {
            ...
            let stackHeight = floor((1080.0 - Config.shared.windowGap) / 2.0)
            check(frames.count == 3, "three windows count")
            check(frames[1].height == stackHeight, "first stack height")
            check(frames[2].origin.y - frames[1].maxY == Config.shared.windowGap, "stack gap height")
            check(frames[2].height == 1080 - stackHeight - Config.shared.windowGap, "last stack height")
        }
        ...
        for count in 4...8 {
            ...
            for i in 1..<stack.count {
                ...
                check(abs(gap - Config.shared.windowGap) < 0.001, "stack gap at count=\(count) i=\(i)")
            }
        }
        ...
        for count in 1...8 {
            let frames = Tiler.calculateFrames(count: count, screen: screen, layout: .tile)
            for frame in frames {
                check(screen.contains(frame), "frame stays inside screen at count=\(count)")
            }
        }
        ...
        do {
            ...
            let stackInset = Config.shared.windowGap - floor(Config.shared.windowGap / 2)
            check(frames[0].origin.x == 100, "offset master x")
            check(frames[0].origin.y == 50, "offset master y")
            check(frames[1].origin.x == 100 + floor(1920 * Config.shared.masterRatio) + stackInset, "offset stack x")
